...
def get_net_fn(trial: optuna.trial.Trial) -> Any:
    l1 = trial.suggest_int("l1", 32, 512)
    l2 = trial.suggest_int("l2", 32, 512)

    def custom_net_fn(batch: Batch) -> jnp.ndarray:
...
def objective(trial: optuna.trial.Trial) -> float:
    net = hk.without_apply_rng(hk.transform(get_net_fn(trial)))
    opt = optax.adam(trial.suggest_float("init_lr", 1e-4, 1, log=True))
